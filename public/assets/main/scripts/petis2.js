...
(function()
{
	...
	var app = function(doc = false)
	{
		...
		* @return   mixed
		*/
		this.on = function(event, callback, useCapture = false)
		{
		 ...
		 * @return    mixed
		 */
		this.create = function(tagName, attribute = false)
		{
		 ...
		 * @return    mixed
		 */
		this.load = function(url, callback = null)
		{
			ajax({
				url: url,
				method: 'GET',
		 ...
		 * @return    mixed
		 */
		this.attr = function(attr, setVal = false)
		{
		 ...
		 * @return    mixed
		 */
		this.style = function(properties, target = false)
		{
		...
		* @return   mixed
		*/
		this.getFormData = function(selector = false)
		{
		...
		*/
		...
		{
			(str === 'on') ? (PETIS.global.debug = 'on') : false;
		};
	...
	PETIS.global = {
		// This 'PETIS.global.string' is most used by some functions, especially custom callable
		string: '',
		// Auto set global document element.
		// The variable is used by most cases in Petis DOM. It's the main global document element in Petis.
		// But, when user using other Petis's services except DOM, the variable and the following
		// objects bellow (in APPLICATION) will not be used or executed.
		elem: document,
		num: 0,
	 ...
	 * @return   mixed
	 */
	PETIS.init = function(rmListener = false)
	{
		var start = function()
		{
			window.Petis = window.petis = function(doc = false)
			{
				...
				if(_isObject(doc))
				{
					if(_isset(doc.ajax) && doc.ajax === true)
					{
						ajax(doc);
					...
					if(_isset(doc.uri) && doc.uri === true)
					{
						return true;
					}

					if(_isset(doc.encode))
					{
						if(_isset(doc.type))
						{
						...
						return PETIS.encode(doc.encode);
					}
				}
				else if(_isFunc(doc))
				{
	...
	PETIS.encode = function(data, usage = 'standart')
	{
		if(usage === 'standart')
		{
	...
	/* Branches */
			// if(browser support let)
			// {
			// 	_getIndex = (string) => string[0];
			// 	_getName = (string, index = false) => (!index ? string.replace(PETIS.global.regex.selector, '')
			// 		: string.replace(index, '').replace(PETIS.global.regex.selector, ''));
			// 	_getRealAttr = (index) => (_issetRealAttr(index) ?
			// 		PETIS.global.list.attribute.real[PETIS.global.list.attribute.fake.indexOf(index)] : index);
			// 	_getError = (string) => ((PETIS.global.debug === 'on') ? string : false);
			// /* Type checking */
			// 	_isString = (data) => (Object.prototype.toString.call(data) === '[object String]');
			// 	_isBoolean = (data) => (Object.prototype.toString.call(data) === '[object Boolean]');
			// 	_isObject = (data) => (((Object.prototype.toString.call(data) === '[object Object]'
			// 		|| typeof data == 'object') && !_isArray(data)) ? true : false);
			// 	_isFunc = (data) => (Object.prototype.toString.call(data) === '[object Function]');
			// 	_isNull = (data) => (Object.prototype.toString.call(data) === '[object Null]');
			// 	_isArray = (data) => ((Array.isArray) ? Array.isArray(data) :
			// 		(Object.prototype.toString.call(data) === '[object Array]'));
			// 	_isHTMLCollection = (data) => (Object.prototype.toString.call(data) === '[object HTMLCollection]');
			// /* Content checking */
			// 	_issetIndex = (array, index) => ((array.indexOf(index) < 0) ? false : true);
			// 	_issetRealAttr = (index) => (_issetIndex(PETIS.global.list.attribute.fake, index) ? true : false);
			// 	_isset = (variable) => ((typeof variable != 'undefined') ? true : false);
			// }
			//
			// @ If you know how to check JavaScript 'let' function for browsers, you can contact <dalikewara@windowslive.com>
			// or fork Petis on GitHub/Bitbucket at github.com/dalikewara/petis or bitbucket.org/dalikewara/petis
			var _getIndex = function(string){ return string[0];},
				_getName = function(string, index = false){ return (!index ? string.replace(PETIS.global.regex.selector, '')
					: string.replace(index, '').replace(PETIS.global.regex.selector, ''));},
				_getRealAttr = function(index){ return (_issetRealAttr(index) ?
					PETIS.global.list.attribute.real[PETIS.global.list.attribute.fake.indexOf(index)] : index);},
				_getError = function(string){ return ((PETIS.global.debug === 'on') ? string : false);},
				...
				_issetRealAttr = function(index){ return (_issetIndex(PETIS.global.list.attribute.fake, index) ? true : false);},
	...
	/* Functions */
		...
		* @return   mixed
		*/
		function on(event, target, callback, useCapture = false)
		{
		 ...
		 * @return    mixed|bool
		 */
		function create(tagName, attribute = false)
		{
		 ...
		 * @return    mixed
		 */
		function attributes(elem, attr, setVal = false, target = false)
		{
		 ...
		 * @return    mixed
		 */
		function styles(elem, properties, target = false)
		{
		...
		* @return   mixed
		*/
		function effects(elem, name, target = false)
		{
			// Initialize call object to handle all main functions.
			// This object variable will contains specified functions or methods.
			var call = {};
		...
		*/
		 ...
		 * @return   string
		 */
		function formData(elem, selector = false)
		{
			var a = 0;
			var len = elem.length;
			...
			if(!selector)
			{
				for( ; a < len; a++)
		        {
		            if(!(PETIS.global.list.form.elemSelector.indexOf(
					Object.prototype.toString.call(elem[a])) < 0))
		            {
		                if(PETIS.global.list.form.specialAttr.indexOf(elem[a].type) < 0 || (!(
						PETIS.global.list.form.specialAttr.indexOf(elem[a].type) < 0)
						&& (elem[a].hasAttribute('checked') || elem[a].hasAttribute('selected'))))
		                {
		                    PETIS.global.string += PETIS.encode(elem[a].name) + '=' +
